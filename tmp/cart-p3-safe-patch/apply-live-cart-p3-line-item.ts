...
const anchor = `  {%- if line_item.image != blank -%}
...
const injected = `  {%- if line_item.image != blank -%}
...
async function main() {
  ...
  if (currentContent.includes("assign line_item_alt")) {
    throw new Error("line_item_alt patch already exists in live snippet.");
  }

  if (!currentContent.includes(anchor)) {
    throw new Error("Expected image_tag anchor not found in live snippet.");
  }
  ...
  const backupPath = path.join(
    backupDir,
    `live-main.snippets__line-item.liquid.before-cart-p3.${timestamp}`,
  );
  ...
  const localPatchedPath = path.join(localPatchedDir, "snippets__line-item.liquid.patched");
